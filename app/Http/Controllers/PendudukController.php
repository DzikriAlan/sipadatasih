...
class PendudukController extends Controller
{
    ...
    public function index()
    {
        if (Session::has('ketua_rt')) {
            $rt = DB::table('pengguna')
                ->join('penduduk', 'penduduk.id', '=', 'pengguna.id_penduduk')
                ->where('pengguna.id', Session::get('id'))
                ->select('pengguna.*', 'penduduk.rt', 'penduduk.rw')
                ->first();
            $daftar_penduduk = Penduduk::orderBy('penduduk.id', 'desc')
            ->where('penduduk.verified', 1)
            ->where('penduduk.rt', $rt->rt)
            ->where('penduduk.rw', $rt->rw)
            ->paginate(25);
        }
        elseif (Session::has('ketua_rw')) {
            $rw = DB::table('pengguna')
                ->join('penduduk', 'penduduk.id', '=', 'pengguna.id_penduduk')
                ->where('pengguna.id', Session::get('id'))
                ->select('pengguna.*', 'penduduk.rw')
                ->first();
            $daftar_penduduk = Penduduk::orderBy('penduduk.id', 'desc')
            ->where('penduduk.verified', 1)
            ->where('penduduk.rw', $rw->rw)
            ->paginate(25);
        }
        else {
            $daftar_penduduk = Penduduk::orderBy('penduduk.id', 'desc')
    ...
    public function verifPenduduk()
    {
        ...
        ->select('pengguna.*', 'penduduk.rt', 'penduduk.rw')
        ->first();
        $daftar_pengguna_belum_terverifikasi = Pengguna::orderBy('id', 'desc')
        ->join('penduduk', 'penduduk.id', '=', 'pengguna.id_penduduk')
        ->where('pengguna.verified', 0)
        ->where('penduduk.rt', $rt->rt)
        ->where('penduduk.rw', $rt->rw)
        ->select('pengguna.id', 'penduduk.nik', 'penduduk.no_kk', 'penduduk.nama_penduduk', 'penduduk.alamat_penduduk', 'penduduk.rt', 'penduduk.rw',
                'penduduk.nik', 'penduduk.alamat_penduduk', 'penduduk.nik', 'penduduk.is_pendatang', 'penduduk.jenis_kelamin')
        ->paginate(25);
        $update_terakhir = Penduduk::orderBy('updated_at', 'desc')->first();


        
        return view('penduduk.verifikasi', compact( 'update_terakhir', 'daftar_pengguna_belum_terverifikasi'));
    }
    ...
    {
        $pengguna = Pengguna::where('verified', 0)->pluck('nama_pengguna', 'id');
        return view('penduduk.create', compact('pengguna'));
    }
    ...
    public function store(PendudukRequest $request)
    {
        ...
        $penduduk = Penduduk::create($input);
        ...
        //     $pengguna->id_penduduk = $penduduk->id;
        //     $pengguna->nama_pengguna = $penduduk->nama_penduduk;

        //     $split = explode(" ", $penduduk->nama_penduduk); // string jadi array
        //     $username = $split[0] . rand(0, 100); // ambil kata pertama dari nama yang diarraykan kemudian ditambah angka

        //     $pengguna->email_pengguna  = $username . "@gmail.com"; // jadiin email
        //     $pengguna->password_pengguna = Hash::make($username); // password sama seperti email
        //     $pengguna->no_hp = rand(100000, 1000000);
        //     $pengguna->foto_pengguna = $penduduk->foto_penduduk;
    ...
    public function cetakDetailPenduduk($id)
    {
        ...
        // save
        Session::flash('pesan', 'Laporan Berhasil Diunduh');
        $pdf->Output('I', "Biodata $penduduk->nama_penduduk .pdf", true);
        return redirect('penduduk');
    }
}
